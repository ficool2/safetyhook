...
public:
    ...
    struct Error {
        ...
        static Error bad_inline_hook(InlineHook::Error err) {
            Error e; e.type = BAD_ALLOCATION; e.inline_hook_error = err;
            return e;
        }
    };
    ...
    static tl::expected<MidHook, Error> create(
    ...
    static tl::expected<MidHook, Error> create(
        Target target, MidHookFn destination_fn, Flags flags = Default) {
        return create(FnPtr(target), destination_fn, flags);
    }
    ...
    template <typename Target>
    static tl::expected<MidHook, Error> create(
        const std::shared_ptr<Allocator>& allocator, Target target, MidHookFn destination_fn, Flags flags = Default) {
        return create(allocator, target, destination_fn, flags);
    }
    ...
    MidHookFn destination() const { return m_destination; }
    ...
    const auto& original_bytes() const { return m_hook.m_original_bytes; }
    ...
    bool enabled() const { return m_hook.enabled(); }
